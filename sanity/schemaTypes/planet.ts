...
export default defineType({
  ...
  fields: [
    ...
    defineField({name: 'alignmentModifier', title: 'Alignment Modifier (dark to light)', type: 'number', initialValue: 0}),
    ...
    defineField({name: 'notableCharacters', title: 'Notable Characters', type: 'array', of: [{type: 'string'}], description: 'Famous characters associated with this planet'}),
    defineField({name: 'keyEvents', title: 'Key Events / Battles', type: 'array', of: [{type: 'string'}], description: 'Notable battles or events that took place here'}),
    defineField({name: 'appearances', title: 'Appearances', type: 'array', of: [{type: 'string'}], description: 'Films or shows where this planet is significant'}),
